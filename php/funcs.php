<?php
	...
	function qryCount($sql)	
	{
		...
		{
    			//Conectando no banco
    			$conperm = mysql_connect($dbhost,$dbuser,$dbpassword)
		or die (Showerro("Não foi possível conectar ao banco de dados","index.php"));

    			$dbperm = mysql_select_db($database, $conperm) or die       (
				Showerro("Nao foi possivel selecionar banco de dados","funcs.php"));
		}
		
		$sql_result = mysql_query($sql, $conperm) or die (Showerro("Nao foi possivel executar a consulta:".$sql, "qryCount"));
		
		$resultado= mysql_num_rows($sql_result); 
	...
	function qryRow($sql, $fieldname)
	{
		...
		$sql_result = mysql_query($sql, $conperm) or die (Showerro("Nao foi possivel executar a consulta:".$sql, "qryRow"));
	...
	function qryExec($sql)
	{
		...
		or die (Showerro("Não foi possível conectar ao banco de dados","index.php"));
    			...
    			$dbperm = mysql_select_db($database, $conperm) or die       (Showerro("Nao foi possivel selecionar banco de dados","funcs.php"));
		}
		
		$sql_result = mysql_query($sql, $conperm) or die (Showerro("Nao foi possivel executar a consulta:".$sql, "QryExec:".$sql ));
		if  (!$sql_result)
		{
			return 0;
		}
	...
	{
	    include('config.php');
	    $sql_result = mysql_query($sql, $conperm) or die (Showerro("Nao foi possivel executar a consulta", "qryReport:".$sql));
	    if  (!$sql_result)
	    {
			return 0;
	    }
		...
		//Resumo
		echo "</td> </tr>";
		$sql_result = mysql_query($sql, $conperm) or die (Showerro("Nao foi possivel executar a consulta", "qryReport"));
		if  (!$sql_result)
		{
				return 0;
			}
	...
	{	
		...
		$sql_result = mysql_query($sql, $conperm) or die (Showerro("Nao foi possivel executar a consulta", "SendEmailtoPerson"));
		if  (!$sql_result)
		{
			return 0;
		}
